  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        token.id = user.id // Add this line to include the user's ID in the token
    ...
    async session({ session, token }) {
      if (session?.user) {
        ...
        session.user.id = token.id as string // Add this line to include the user's ID in the session
        if (token.role === "COACH") {
